...
export class HomePage {
  ...
  loadAllQuestions(){
    ...
    // this.dbProvider.getAllQuestions().then(data => {
    //   env.developers = data;
    //   console.log(data);
    //   for(let dev of env.developers)
    //   {
    //     // console.log(dev.firstname + "->" + dev.lastname + "->" + dev.email);
    //     console.log(dev.Event_ID + "->" + "->" + dev.PARTICIPANTS_ID + "->" + dev.QUESTION_ID + "->" + dev.QUESTIONS + "->" + dev.OPTIONS);
    //     env.questionsArray.push({
    ...
    //   env.navCtrl.push(ParticipantListPage,{questions:JSON.stringify(env.questionsArray),eventName:env.eventName},{});
    // }).catch(error=>{
    //   console.log("Home.ts error->" + error)
    // });
  }
